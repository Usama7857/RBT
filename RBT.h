...
private:
	...
	int Search_IN_Tree(Node* temp, int n, int& check)
	{
		if (temp != nullptr)
		{
			if (n < temp->data)
			{
				...
				Search_IN_Tree(temp->leftChild, n, check);
			}
			else
			{
				cout << temp->data << "->";
				...
				Search_IN_Tree(temp->rightChild, n, check);
			}
		}
	}
...
public:
	...
	void displayParent(int n)
	{
		...
		{
			cout << "\t\t\t\t\t\t";
			cout << "Given NOde does not exsit in the tree " << endl;
		}
	...
	{
		int check = 0;
		check = Search_IN_Tree(root, n, check);
		if (check == 0)
		{
			cout << "Given number does not exist in Red Black Tree " << endl << endl;
		}
		else
		{
			cout << "Given number exists in Red Black Tree " << endl << endl;
		}
